...
class Schedule():
    ...
    def __init__(self, year, month, number_of_weeks):
        self.year = year
        self.month = month
        self.calendar = [[None]*7 for _ in range(number_of_weeks)]


def new_schedule(yml_dict):
    month = monthinfo.CurrentMonth(
        yml_dict["month"], yml_dict["year"], 5).calendar()
    return Schedule(yml_dict["year"], yml_dict["month"], len(month))
